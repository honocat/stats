...
以下のシミュレーションで、処置後変数バイアス（特に、媒介変数によるバイアス）を確認する。
...
ここで、$x$と$z$の相関関係を確認する。

```{r}
with(df, cor(x, z))
```

$x$と$z$の間に正の相関があることがわかる。また、$y$と$z$については、
...
推定値を見ると、`x`の係数$\beta$は過小推定されている。

ここで、説明変数$z$を除外して、以下のモデルを考える。
...
統制すべきと考えていた$z$を除外したモデルなのに、$\beta$の推定値は設定したあたいである$0.25$に近い。これは偶然か？

1回シミュレーションを実行し、処置後変数を含むモデルを含まないモデルの係数の推定値を返す関数を作る。
...
このように、ある変数$x$の効果を推定したい時は、その変数の結果として出てくる変数を**統制してはいけない**。変数間に時間的な前後関係があれば、このバイアスを回避するのは比較的容易である。しかし、時間的な前後関係が不明なとき、ある変数が交絡変数か処置後変数かを見抜くのは難しい場合がある。統計モデルを作るときには、自分が統制する変数は交絡であり、処置後変数ではないことを理論的に示す必要がある。
